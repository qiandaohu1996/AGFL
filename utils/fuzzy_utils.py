...
class CosineAnnealingFuzzyMScheduler(FuzzyMScheduler):
    ...
    def step(self, current_round):
        ...
        self.fuzzy_m = self.initial_fuzzy_m + (0.5 * self.smooth_factor * (self.initial_fuzzy_m - self.min_fuzzy_m)) * (np.cos(np.pi * (current_round / self.T_max) ) - 1)
...
def get_fuzzy_m_scheduler( initial_fuzzy_m,scheduler_name="constant",   min_fuzzy_m=1.5, n_rounds=None):
    ...
    if scheduler_name == "sqrt":
        ...
        return CosineAnnealingFuzzyMScheduler(initial_fuzzy_m, T_max=n_rounds//8, min_fuzzy_m=min_fuzzy_m)

    elif scheduler_name == "multi_step":
        assert n_rounds is not None, "Number of rounds is needed for \"multi_step\" scheduler!"
        milestones = [n_rounds//4, 3*(n_rounds//4), n_rounds//2, 5*(n_rounds//8), 3*(n_rounds//4), 7*(n_rounds//8)]
        ...
        return TwoMultiStepFuzzyMScheduler(initial_fuzzy_m, milestones1=milestones1, milestones2=milestones2, gamma=0.1, min_fuzzy_m=min_fuzzy_m)
